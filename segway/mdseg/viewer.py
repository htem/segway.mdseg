...
from .defines import PREDEFINED_TAGS
from .defines import PREDEFINED_CELL_TYPES
from .defines import PREAPPROVED_ANNOTATORS
from .defines import PREDEFINED_CELL_SEGMENT_TYPES
...
class MDSegViewer():
    ...
    def copy_selections(self, s):
        selected = MDSegViewer._get_viewer_segments(s)
    ...
    def paste_selections(self, s, cut=False):
        selected = list(MDSegViewer._get_viewer_segments(s))
    ...
    def _get_selected_segment(self, s):
        # handle the case where there is a separate 'mesh' layer
        # use value from 'mesh' to get the more accurate selection
        try:
            return int(s.selected_values['mesh'].value)
        except:
            pass
        try:
            return int(s.selected_values['seg'].value)
        except:
            pass
        return None

    def add_blacklist(self, s):
        seg_id = self._get_selected_segment(s)
    ...
    def join_selections(self, s):
        ...
        selected_segs = MDSegViewer._get_viewer_segments(s)
    ...
    def _get_state_from_neuron(self, neuron):
        ...
        voxel_size_xyz = [16, 16, 40]
        soma_xyz_loc = [
                str(int(int(soma_zyx_loc['x'])/voxel_size_xyz[0])),
                str(int(int(soma_zyx_loc['y'])/voxel_size_xyz[1])),
                str(int(int(soma_zyx_loc['z'])/voxel_size_xyz[2])),
        ]
    ...
    def set_color(self, s):
        ...
        selections = MDSegViewer._get_viewer_segments(s)
        if len(selections) == 0:
    ...
    def clNeuronColorButton(self, s):
        ...
        if also_load_neurons and not clear_before_load:
            total_segs = MDSegViewer._get_viewer_segments(s)
        ...
        for line in color_mapping_split:
            ...
            if name.split('.')[-1] == 'axon':
                ...
                print(f'checking {subsegment_name}')
                while self.neuron_db.exists_neuron(subsegment_name):
                    print(f'found {subsegment_name}')
